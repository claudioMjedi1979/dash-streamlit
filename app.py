...
st.set_page_config(page_title='Dash Campanha Facebook',
                   layout='wide',
...
with header_mid:
    st.title('Dash Campanha Facebook')
...
with Q1:
        df3 = df1.groupby(by=['campaign']).sum(numeric_only=True)[['Impressions', 'Clicks']].reset_index()
        df3['CTR'] = round(df3['Clicks'] / df3['Impressions'] * 100, 3)
        fig_CTR_by_campaign = px.bar(df3, x='campaign', y='CTR', title='<b>Taxa de cliques</b>')
...
with Q2:
        fig_impressions_per_day = px.line(df1, x='date', y=['Impressions'], color='campaign', title='<b>Impressões diárias por campanha</b>')
...
with Q3:
        df4 = df1.groupby(by='gender').sum(numeric_only=True)[['Spent']].reset_index()
        fig_spend_by_gender = px.pie(df4, names='gender', values='Spent', title='<b>Gastos com publicidade por gênero</b>')
...
with Q4:
        df5 = df1.groupby(by='age').sum(numeric_only=True)[['Spent', 'Total_Conversion']].reset_index()
        df5['CPC'] = round(df5['Spent'] / df5['Total_Conversion'], 2)
        fig_CPC_by_age = px.bar(df5, x='age', y='CPC', title='<b>Custo por conversão por idade demográfica</b>')
        fig_CPC_by_age.update_layout(title={'x': 0.5},
                                     xaxis=(dict(showgrid=False)),
                                     yaxis=(dict(showgrid=False)),
                                     plot_bgcolor="rgba(0,0,0,0)")
